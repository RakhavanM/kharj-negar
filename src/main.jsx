...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="page-content">
      ...
      <section className="month-toolbar"><div className="month-selector"><Icon name="calendar" size={18} /><select aria-label="انتخاب ماه" value={selectedMonth} onChange={(event) => setSelectedMonth(event.target.value)}>{monthOptions.map((month) => <option value={month.value} key={month.value}>{month.label}</option>)}</select><Icon name="chevron" size={17} /></div><div className="toolbar-actions"><button className={`filter-button ${Object.values(filters).some((value) => value !== 'all' && value !== '') ? 'has-filter' : ''}`} onClick={() => setShowFilters((open) => !open)}><Icon name="filter" size={18} />فیلترها</button><button className="mobile-add" aria-label="ثبت هزینه جدید" onClick={openAdd}><Icon name="plus" size={22} /></button></div></section>
