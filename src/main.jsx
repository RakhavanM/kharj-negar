...
function Icon({ name, size = 20 }) {
  const paths = {
    ...
    settings: <><path d="M12 8.5a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Z" /><path d="m19.4 15 .1.1a1.8 1.8 0 0 1-2.5 2.5l-.1-.1a1.8 1.8 0 0 0-3.1 1.3v.2a1.8 1.8 0 0 1-3.6 0v-.2a1.8 1.8 0 0 0-3.1-1.3l-.1.1a1.8 1.8 0 1 1-2.5-2.5l.1-.1a1.8 1.8 0 0 0-1.3-3.1h-.2a1.8 1.8 0 0 1 0-3.6h.2a1.8 1.8 0 0 0 1.3-3.1l-.1-.1a1.8 1.8 0 1 1 2.5-2.5l.1.1a1.8 1.8 0 0 0 3.1-1.3v-.2a1.8 1.8 0 0 1 3.6 0v.2a1.8 1.8 0 0 0 3.1 1.3l.1-.1a1.8 1.8 0 1 1 2.5 2.5l-.1.1a1.8 1.8 0 0 0 1.3 3.1h.2a1.8 1.8 0 0 1 0 3.6h-.2a1.8 1.8 0 0 0-1.3 3.1Z" /></>,
  };
...
function App() {
  ...
  const [categoryCatalog, setCategoryCatalog] = useState(() => DEFAULT_CATEGORIES);
  ...
  const categoryLabels = useMemo(() => ({ ...CATEGORY_LABELS, ...getCategoryLabels(categoryCatalog) }), [categoryCatalog]);
  const categoryIcons = useMemo(() => getCategoryIcons(categoryCatalog), [categoryCatalog]);
  const categoryCodes = useMemo(() => [...new Set([...Object.values(CATEGORIES), ...categoryCatalog.map((category) => category.code)])], [categoryCatalog]);
  const visibleExpenses = previewMode ? PREVIEW_EXPENSES : expenses;
  const demoCategoriesWithUsage = useMemo(() => categoryCatalog.map((category) => ({
    ...category,
    in_use: visibleExpenses.some((expense) => expense.category === category.code),
  })), [categoryCatalog, visibleExpenses]);
  ...
  const reloadCategories = useCallback(async () => {
    if (production) {
      const response = await apiListCategories(true);
      const categories = mergeCategories(response.items);
      setCategoryCatalog(categories);
      return categories;
    }
    const stored = readStorage('kharj-negar-categories-v1', DEFAULT_CATEGORIES);
  ...
  useEffect(() => {
    if (session) reloadCategories().catch((error) => production && setServerError(error.message));
  }, [production, session, reloadCategories]);
  ...
  const localSummary = useMemo(() => getMonthSummary(visibleExpenses, selectedMonth, filters, categoryCodes), [visibleExpenses, selectedMonth, filters, categoryCodes]);
  ...
  const summary = production
    ? (serverData.summary ? {
      ...
      byCategory: { ...Object.fromEntries(categoryCodes.map((code) => [code, 0])), ...serverData.summary.by_category },
      comparison: serverData.summary.comparison,
    } : { total: 0, count: 0, byPerson: { ramin: 0, mana: 0 }, byCategory: Object.fromEntries(categoryCodes.map((code) => [code, 0])), comparison: null })
  ...
  const refreshCategoryCatalog = async () => {
    setCategoryManagerLoading(true);
    try {
      const categories = await reloadCategories();
      setCategoryCatalog(categories);
      if (!production) window.localStorage.setItem('kharj-negar-categories-v1', JSON.stringify(categories));
      return categories;
    } finally {
      setCategoryManagerLoading(false);
    }
  };
  const saveCategory = async ({ id, name, version }) => {
    const normalized = normalizeCategoryName(name);
    if (!normalized) throw new Error('نام دسته‌بندی نمی‌تواند خالی باشد.');
    if (categoryNameExists(categoryCatalog, normalized, id ? categoryCatalog.find((item) => String(item.id) === String(id) || item.code === id)?.code : '')) {
      throw new Error('این دسته‌بندی قبلاً وجود دارد.');
    }
    const existing = id ? categoryCatalog.find((item) => String(item.id) === String(id) || item.code === id) : null;
    const response = production
      ? (id ? await apiUpdateCategory(id, normalized, version) : await apiCreateCategory(normalized))
      : { ...(existing || {}), id: id || `local-${crypto.randomUUID()}`, code: existing?.code || `custom-${crypto.randomUUID()}`, name: normalized, is_active: true, sort_order: existing?.sort_order || Math.max(0, ...categoryCatalog.map((item) => item.sort_order || 0)) + 10, version: id ? (version || 1) + 1 : 1, in_use: existing?.in_use || false };
    const next = id ? categoryCatalog.map((item) => (String(item.id) === String(id) || item.code === id) ? response : item) : [...categoryCatalog, response];
    setCategoryCatalog(next);
    if (!production) window.localStorage.setItem('kharj-negar-categories-v1', JSON.stringify(next));
    if (filters.category === id) setFilters((current) => ({ ...current, category: response.code }));
  };
  const archiveCategory = async (category) => {
    if (production) await apiDeleteCategory(category.id);
    const next = categoryCatalog.map((item) => item.code === category.code ? { ...item, is_active: false, version: (item.version || 1) + 1 } : item);
    setCategoryCatalog(next);
    if (!production) window.localStorage.setItem('kharj-negar-categories-v1', JSON.stringify(next));
    if (filters.category === category.code) setFilters((current) => ({ ...current, category: 'all' }));
  };
  const restoreCategory = async (category) => {
    if (production) await apiRestoreCategory(category.id);
    const next = categoryCatalog.map((item) => item.code === category.code ? { ...item, is_active: true, version: (item.version || 1) + 1 } : item);
    setCategoryCatalog(next);
    if (!production) window.localStorage.setItem('kharj-negar-categories-v1', JSON.stringify(next));
  };
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark"><img src={BRAND_LOGO_PATH} alt="" /></div><div><strong>خرج‌نگار</strong><small>هزینه‌های خونه</small></div></div><div className="topbar-greeting">سلام {PERSON_LABELS[session.person]}<span aria-hidden="true">!</span></div><div className="topbar-actions"><ThemeToggle theme={theme} onToggle={toggleTheme} />{production && <button className="download-button" type="button" onClick={() => setDownloadConfirmationOpen(true)} disabled={exporting} title={DOWNLOAD_CONFIRMATION_LABEL} aria-label={DOWNLOAD_CONFIRMATION_LABEL}><Icon name="download" size={16} /></button>}<div className="settings-wrap"><button className="settings-button" type="button" onClick={() => setSettingsOpen((open) => !open)} title="تنظیمات" aria-label="تنظیمات" aria-haspopup="menu" aria-expanded={settingsOpen}><Icon name="settings" size={17} /></button>{settingsOpen && <div className="settings-menu" role="menu" aria-label="تنظیمات"><button type="button" role="menuitem" onClick={() => { setSettingsOpen(false); setPasswordModalOpen(true); }}>تغییر پسورد</button><button type="button" role="menuitem" onClick={() => { setSettingsOpen(false); setCategoryManagerError(''); setCategoryManagerOpen(true); refreshCategoryCatalog().catch((error) => setCategoryManagerError(error.message)); }}>تعریف دسته‌بندی</button></div>}</div><button className="logout-button" onClick={async () => { if (production) await apiLogout(); setSession(null); }} title="خروج از حساب"><Icon name="logout" size={17} /></button></div></header>
    ...
    {activeSection === 'expenses' ? <main className="page-content">
      <section className="welcome-row"><div><h1 className="expenses-title">هزینه‌های ما</h1><p className="muted">هزینه‌ها را ثبت کنید تا تصویر روشن‌تری از خرج‌های خانه داشته باشید.</p></div><button className="primary-button desktop-add" onClick={openAdd}><Icon name="plus" size={20} />ثبت هزینه جدید</button></section>
      ...
      {activeView === 'dashboard' ? <Dashboard summary={summary} expenses={filteredExpenses} comparison={summary.comparison} onEdit={openEdit} onDelete={deleteExpense} onShowAll={() => setActiveView('expenses')} categories={categoryCodes} categoryLabels={categoryLabels} categoryIcons={categoryIcons} /> : activeView === 'expenses' ? <ExpensesList expenses={filteredExpenses} onEdit={openEdit} onDelete={deleteExpense} categoryLabels={categoryLabels} categoryIcons={categoryIcons} /> : <SavingsView assets={visibleSavingsAssets} loading={savingsLoading} error={savingsError} onAdd={openSavingsAdd} onEdit={openSavingsEdit} onDelete={deleteSavingsAsset} />}
    ...
    <nav className="bottom-nav" aria-label="عملیات اصلی"><button className="bottom-action" onClick={activeSection === 'expenses' ? openAdd : openSavingsAdd}><span><Icon name={activeSection === 'expenses' ? 'plus' : 'wallet'} size={22} /></span><label>{activeSection === 'expenses' ? 'ثبت هزینه' : 'افزودن دارایی'}</label></button></nav>
    {isFormOpen && <ExpenseModal expense={editingExpense} onSave={saveExpense} onClose={() => { setFormOpen(false); setEditingExpense(null); }} categories={categoryCatalog} />}
    ...
    {categoryManagerOpen && <CategoryManager categories={production ? categoryCatalog : demoCategoriesWithUsage} loading={categoryManagerLoading} error={categoryManagerError} onSave={saveCategory} onArchive={archiveCategory} onRestore={restoreCategory} onRefresh={refreshCategoryCatalog} onClose={() => setCategoryManagerOpen(false)} />}
...
function Dashboard({ summary, expenses, comparison, onEdit, onDelete, onShowAll, categories, categoryLabels, categoryIcons }) {
  const categoryItems = categories.map((category) => ({ category, amount: summary.byCategory[category] || 0 })).filter((item) => item.amount > 0).sort((a, b) => b.amount - a.amount);
  return <><section className="content-grid"><div className="panel category-panel"><div className="panel-heading"><div><p className="eyebrow">تقسیم‌بندی</p><h2>هزینه‌ها کجا رفته‌اند؟</h2></div><Icon name="chart" size={22} /></div>{categoryItems.length ? <div className="category-list">{categoryItems.slice(0, 5).map(({ category, amount }) => <div className="category-row" key={category}><span className="category-icon">{categoryIcons[category]}</span><div className="category-meta"><div><strong>{categoryLabels[category]}</strong><span>{getCategoryPercentage(amount, summary.total)}٪</span></div><div className="category-bar"><span style={{ width: `${getCategoryPercentage(amount, summary.total)}%` }} /></div></div><b>{formatNumber(amount)}</b></div>)}</div> : <EmptyState text="هنوز هزینه‌ای در این ماه ثبت نشده است." />}</div><div className="panel recent-panel"><div className="panel-heading"><div><p className="eyebrow">آخرین فعالیت‌ها</p><h2>هزینه‌های اخیر</h2></div><button className="text-button" onClick={onShowAll}>مشاهده همه <Icon name="chevron" size={15} /></button></div>{expenses.length ? <div className="recent-list">{expenses.slice(0, 4).map((expense) => <ExpenseRow key={expense.id} expense={expense} onEdit={onEdit} onDelete={onDelete} categoryLabels={categoryLabels} categoryIcons={categoryIcons} />)}</div> : <EmptyState text="هزینه‌ای مطابق فیلترهای فعلی پیدا نشد." />}</div></section><section className="insight-strip"><div className="insight-mark">✦</div><div><strong>یک نگاه سریع</strong><p>{categoryItems[0] ? `بیشترین سهم این ماه مربوط به «${categoryLabels[categoryItems[0].category]}» است.` : 'با ثبت اولین هزینه، خلاصه وضعیت ماه اینجا نمایش داده می‌شود.'}</p>{comparison?.available && <p className={`month-comparison ${comparison.direction}`}>{comparison.direction === 'less' ? `تا امروز ${comparison.percent}٪ کمتر از ماه قبل خرج شده.` : comparison.direction === 'more' ? `تا امروز ${comparison.percent}٪ بیشتر از ماه قبل خرج شده.` : 'هزینه‌ها نسبت به ماه قبل تغییری نکرده‌اند.'}</p>}</div><span className="insight-value">{categoryItems[0] ? `${getCategoryPercentage(categoryItems[0].amount, summary.total)}٪` : '—'}</span></section><CategoryPieChart summary={summary} categories={categories} categoryLabels={categoryLabels} /> </>;
}
function ExpensesList({ expenses, onEdit, onDelete, categoryLabels, categoryIcons }) { return <section className="panel expenses-panel"><div className="panel-heading"><div><p className="eyebrow">فهرست تراکنش‌ها</p><h2>همه هزینه‌ها</h2></div><span className="result-count">{expenses.length} مورد</span></div>{expenses.length ? <div className="expenses-table">{expenses.map((expense) => <ExpenseRow key={expense.id} expense={expense} onEdit={onEdit} onDelete={onDelete} categoryLabels={categoryLabels} categoryIcons={categoryIcons} detailed />)}</div> : <EmptyState text="هزینه‌ای مطابق فیلترهای فعلی پیدا نشد." />}</section>; }

function SavingsView({ assets, loading, error, onAdd, onEdit, onDelete }) { return <section className="savings-section"><div className="savings-heading"><div><h2>پس‌اندازهای ما</h2><p className="muted">موجودی دارایی‌ها جدا از هزینه‌های روزمره نگهداری می‌شود.</p></div><button className="primary-button" onClick={onAdd}><Icon name="plus" size={18} />افزودن دارایی</button></div>{error && <p className="form-error">{error}</p>}{loading ? <EmptyState text="در حال دریافت دارایی‌ها..." /> : assets.length ? <div className="savings-grid">{assets.map((asset) => <SavingsCard asset={asset} key={asset.id} onEdit={onEdit} onDelete={onDelete} />)}</div> : <EmptyState text="هنوز دارایی‌ای ثبت نشده است." />}</section>; }
...
function CategoryPieChart({ summary, categories, categoryLabels }) { const items = categories.map((category) => ({ category, amount: summary.byCategory[category] || 0 })).filter((item) => item.amount > 0); const total = items.reduce((sum, item) => sum + item.amount, 0); const colors = ['var(--chart-1)', 'var(--chart-2)', 'var(--chart-3)', 'var(--chart-4)', 'var(--chart-5)', 'var(--chart-6)', 'var(--chart-7)', 'var(--chart-8)']; let cursor = 0; const segments = items.map((item, index) => { const start = cursor; cursor += (item.amount / total) * 100; return `${colors[index % colors.length]} ${start}% ${cursor}%`; }); return <section className="panel pie-panel"><div className="panel-heading"><div><p className="eyebrow">تصویر هزینه‌ها</p><h2>سهم هر دسته‌بندی</h2></div><Icon name="chart" size={22} /></div>{items.length ? <div className="pie-layout"><div className="pie-chart" style={{ background: `conic-gradient(${segments.join(', ')})` }} role="img" aria-label="نمودار سهم دسته‌بندی‌های هزینه" /><div className="pie-legend">{items.map((item, index) => <div className="pie-legend-row" key={item.category}><span style={{ background: colors[index % colors.length] }} /><strong>{categoryLabels[item.category] || item.category}</strong><b>{getCategoryPercentage(item.amount, total)}٪</b></div>)}</div></div> : <EmptyState text="برای نمایش نمودار، ابتدا هزینه‌ای ثبت کنید." />}</section>; }

function CategoryManager({ categories, loading, error, onSave, onArchive, onRestore, onRefresh, onClose }) {
  ...
  const submit = async (event) => {
    ...
    setLocalError('');
    try {
      await onSave({ id: editing?.id, name, version: editing?.version });
      setName('');
      setEditing(null);
    } catch (saveError) {
  ...
  const edit = (category) => { setEditing(category); setName(category.name); setLocalError(''); };
  const archive = async (category) => {
    if (active.length <= 1) { setLocalError('حداقل یک دسته‌بندی فعال باید باقی بماند.'); return; }
    if (!window.confirm(`دسته‌بندی «${category.name}» غیرفعال شود؟ سوابق هزینه‌ها حفظ می‌شوند.`)) return;
    try { await onArchive(category); } catch (archiveError) { setLocalError(archiveError.message); }
  };
  const restore = async (category) => {
    try { await onRestore(category); } catch (restoreError) { setLocalError(restoreError.message); }
  };
  const renderRow = (category, archived = false) => <div className={`category-manager-row ${archived ? 'is-archived' : ''}`} key={category.code}><div><strong>{category.name}</strong>{archived && <span>غیرفعال</span>}{category.in_use && <small>دارای سابقه هزینه</small>}</div><div><button type="button" className="text-button" onClick={() => edit(category)}>ویرایش</button>{archived ? <button type="button" className="text-button" onClick={() => restore(category)}>فعال‌سازی مجدد</button> : <button type="button" className="text-button danger-text" onClick={() => archive(category)}>حذف</button>}</div></div>;
  return <div className="modal-backdrop" role="presentation"><section className="category-manager" role="dialog" aria-modal="true" aria-labelledby="category-manager-title"><div className="modal-head"><div><p className="eyebrow">مدیریت هزینه‌ها</p><h2 id="category-manager-title">تعریف دسته‌بندی</h2></div><button type="button" onClick={onClose} aria-label="بستن"><Icon name="close" size={21} /></button></div><form className="category-manager-form" onSubmit={submit}><label>نام دسته‌بندی<input value={name} onChange={(event) => setName(event.target.value)} placeholder="مثلاً خرج روزمره" maxLength="80" /></label><div className="category-manager-form-actions"><button type="submit" className="primary-button" disabled={loading}>{editing ? 'ذخیره ویرایش' : 'افزودن'}</button>{editing && <button type="button" className="secondary-button" onClick={() => { setEditing(null); setName(''); }}>انصراف</button>}</div></form>{(localError || error) && <p className="form-error">{localError || error}</p>}<div className="category-manager-list"><div className="category-manager-list-head"><strong>دسته‌بندی‌های فعال</strong><button type="button" className="text-button" onClick={onRefresh}>بروزرسانی</button></div>{loading ? <EmptyState text="در حال دریافت دسته‌بندی‌ها..." /> : active.map((category) => renderRow(category))}</div>{inactive.length > 0 && <div className="category-manager-list archived-list"><strong>دسته‌بندی‌های غیرفعال</strong>{inactive.map((category) => renderRow(category, true))}</div>}</section></div>;
}
