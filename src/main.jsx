...
function App() {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand-lockup"><div className="brand-mark"><span>خ</span></div><div><strong>خرج‌نگار</strong><small>هزینه‌های خونه</small></div></div><div className="topbar-actions"><ThemeToggle theme={theme} onToggle={toggleTheme} />{production && <button className="download-button" type="button" onClick={() => setDownloadConfirmationOpen(true)} disabled={exporting} title={DOWNLOAD_CONFIRMATION_LABEL} aria-label={DOWNLOAD_CONFIRMATION_LABEL}><Icon name="download" size={16} /></button>}<button className="profile-button" type="button" onClick={() => setPasswordModalOpen(true)} title="تغییر رمز عبور"><span>{PERSON_LABELS[session.person][0]}</span><Icon name="lock" size={17} /></button><button className="logout-button" onClick={async () => { if (production) await apiLogout(); setSession(null); }} title="خروج از حساب"><Icon name="logout" size={17} /></button></div></header>
